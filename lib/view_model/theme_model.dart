...
import 'package:flutter_app_funandroid/ui/helper/theme_helper.dart';
...
/// 明暗模式
  ...
  ThemeModel(){
    ...
    _brightness = Brightness.values[
      StorageManager.sharedPreference.get(kThemeBrightnessIndex)??0
    ];
    ///获取主题颜色
    _themeColor = Colors.primaries[
      StorageManager.sharedPreference.getInt(kThemeColorIndex) ??0
    ];
...
/// 没有传[brightness]就不改变brightness,color同理
  ...
  _generateThemeData(){
    ...
    var themeData = ThemeData(
      brightness: _brightness,
      // 主题颜色属于亮色系还是属于暗色系
      // (eg:dark时,AppBarTitle文字及状态栏文字的颜色为白色,反之为黑色)
...
/// 数据持久化到shared preferences
  saveTheme2Storage(Brightness brightness,MaterialColor themeColor)async{
    ...
    await Future.wait([
      StorageManager.sharedPreference.setInt(kThemeBrightnessIndex
          , brightness.index),
